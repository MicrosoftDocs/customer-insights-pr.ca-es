...
ms.locfileid: "7920863"
---
# <a name="log-forwarding-in-dynamics-365-customer-insights-with-azure-monitor-preview"></a>Inicia la sessió Dynamics 365 Customer Insights amb l'Azure Monitor (Preview)

Dynamics 365 Customer Insights proporciona una integració directa amb l'Azure Monitor. Els registres de recursos de l'Azure Monitor us permeten supervisar i enviar registres a [l'Azure Storage](https://azure.microsoft.com/services/storage/), Azure Log Analytics, o [transmetre](/azure/azure-monitor/logs/log-analytics-overview)'ls als [concentradors d'esdeveniments de l'Azure](https://azure.microsoft.com/services/event-hubs/).

Customer Insights envia els registres d'incidències següents:

- **Esdeveniments d'auditoria**
  - **APIEvent** : permet el seguiment de canvis realitzat a través de la interfície Dynamics 365 Customer Insights d'usuari.
- **Esdeveniments operatius**
  - **WorkflowEvent** : El flux de treball permet configurar [orígens de](data-sources.md) dades, [unificar i enriquir](data-unification.md)[i](enrichment-hub.md), finalment, [exportar dades a altres](export-destinations.md) sistemes. Tots aquests passos es poden fer individualment (per exemple, activar una sola exportació) o orquestrats (per exemple, l'actualització de dades de fonts de dades que desencadenen el procés d'unificació que provocarà enriquiments addicionals i, un cop fetes, exportar les dades a un altre sistema). Per obtenir més detalls, vegeu [l'esquema WorkflowEvent](#workflow-event-schema).
  - **APIEvent:** totes les trucades de l'API a la instància dels clients a Dynamics 365 Customer Insights. Per a més detalls, vegeu [l'esquema APIEvent](#api-event-schema).
...
- Teniu la **funció** d'administrador de col·laboradors i **d'accés** a l'usuari al recurs de destinació de l'Azure. El recurs pot ser un compte d'emmagatzematge de l'Azure, un centre d'esdeveniments de l'Azure o una àrea de treball de l'Azure Log Analytics. Per obtenir més informació, vegeu [Afegeix o suprimeix assignacions de funcions de l'Azure mitjançant el portal de l'Azure](/azure/role-based-access-control/role-assignments-portal).
- [S'han complert els requisits](/azure/azure-monitor/platform/diagnostic-settings#destination-requirements) de destinació per a l'Azure Storage, l'Azure Event Hub o l'Azure Log Analytics.
...
1. Aneu al **diagnòstic del** > **sistema**.
...
- **Esdeveniments** operatius: [esdeveniments de](#api-event-schema)[l'API o esdeveniments de flux de treball](#workflow-event-schema) generats mentre s'utilitza el servei.  Per exemple, `GET` sol·licituds o els esdeveniments d'execució d'un flux de treball.
...
Les incidències de l'API i els esdeveniments del flux de treball tenen una estructura i detalls comuns en què difereixen, vegeu [l'esquema d'esdeveniments de l'API](#api-event-schema) o [l'esquema d'esdeveniments del flux de treball](#workflow-event-schema).
...
| `category`        | String    | Necessari          | Registra la categoria de l'esdeveniment. O `Operational``Audit` o. Totes les sol·licituds HTTP POST/PUT/PATCH/DELETE estan etiquetades amb `Audit`, tota la resta amb`Operational` | `2020-09-08T09:48:14.8050869Z`                                                                                                                                           |
...
| `identity`        | String    | Opcional          | Objecte JSON que descriu la identitat de l'usuari o aplicació que va fer l'operació.       | Vegeu [la](#identity-schema) secció Identitat.     |  |
| `properties`      | String    | Opcional          | Objecte JSON amb més propietats a la categoria particular d'esdeveniments.      | Vegeu [la](#api-properties-schema) secció Propietats.    |
...
`identity` L'objecte JSON té la següent estructura
...
| Camp                         | Descripció                                                                                                                          |
| ----------------------------- | ------------------------------------------------------------------------------------------------------------------------------------ |
| `Authorization.UserRole`      | Funció assignada per a l'usuari o l'aplicació. Per obtenir més informació, vegeu [permisos d'usuari](permissions.md).                                     |
...
[Les incidències de](apis.md) l'API tenen les propietats següents.
...
El flux de treball conté diversos passos. [Ingereix fonts de](data-sources.md) dades, [unifica,](data-unification.md)[enriqueix](enrichment-hub.md) i [exporta](export-destinations.md) dades. Tots aquests passos poden executar-se individualment o orquestrats amb els següents processos. 
...
| `time`          | Data i hora | Necessari          | Marca horària de l'esdeveniment (UTC).                                                                                                                                 | `2020-09-08T09:48:14.8050869Z`                                                                                                                                           |
| `resourceId`    | String    | Necessari          | ResourceId de la instància que ha emès l'esdeveniment.                                                                                                            | `/SUBSCRIPTIONS/XXXXXXXX-XXXX-XXXX-XXXX-XXXXXXXX/RESOURCEGROUPS/<RESOURCEGROUPNAME>/`<br>`PROVIDERS/MICROSOFT.D365CUSTOMERINSIGHTS/`<br>`INSTANCES/XXXXXXXX-XXXX-XXXX-XXXX-XXXXXXXX` |
| `operationName` | String    | Necessari          | Nom de l'operació representada per aquest esdeveniment. `{OperationType}.[WorkFlow|Task][Started|Completed]`. Vegeu [Tipus d'operacions](#operation-types) per a la referència. | `Segmentation.WorkflowStarted`,<br> `Segmentation.TaskStarted`, <br> `Segmentation.TaskCompleted`, <br> `Segmentation.WorkflowCompleted`                                 |
...
| `properties.submittedBy`                     | Sí      | No   | Opcional. Només les incidències del flux de treball. Azure Active Directory [L'objectId de l'usuari](/azure/marketplace/find-tenant-object-id#find-user-object-id) que ha activat el flux de treball, vegeu també `properties.workflowSubmissionKind`.                                   |
...
| `properties.additionalInfo.Kind`             | No       | Sí  | Opcional. Només per a `Export` OperationType. Identifica el tipus d'exportació. Per obtenir més informació, vegeu [la visió general de les destinacions d'exportació](export-destinations.md).                                                                                          |
| `properties.additionalInfo.AffectedEntities` | No       | Sí  | Opcional. Només per a `Export` OperationType. Conté una llista d'entitats configurades a l'exportació.                                                                                                                                                            |
| `properties.additionalInfo.MessageCode`      | No       | Sí  | Opcional. Només per a `Export` OperationType. Missatge detallat per a l'exportació.                                                                                                                                                                                 |
| `properties.additionalInfo.entityCount`      | No       | Sí  | Opcional. Només per a `Segmentation` OperationType. Indica el nombre total de membres que té el segment.                                                                                                                                                    |
